...
将代码推送到 GitHub 后，在 **Actions → DNSHE domain renewal → Run workflow** 手动运行一次验证。手动运行时可填写一个或多个完整域名（英文逗号分隔），留空则处理全部；手动输入会优先于 `DNSHE_DOMAINS` Variable。之后会在每月 1 日 **08:15（北京时间）** 自动运行一次。可在 [`.github/workflows/dnshe-renew.yml`](.github/workflows/dnshe-renew.yml) 修改 Cron；GitHub 的定时任务有时会有排队延迟。

...
该项目每天通过 DNSHE Free Domain API 尝试续期，并将每个域名的处理结果推送到 Bark。续期尚未开放会显示为 `not_due`，不会使工作流失败；其他 API 失败会使工作流失败并以 critical 级别通知。
...
将代码推送到 GitHub 后，在 **Actions → DNSHE domain renewal → Run workflow** 手动运行一次验证。之后会在每天 **08:15（北京时间）** 自动运行一次。可在 [`.github/workflows/dnshe-renew.yml`](.github/workflows/dnshe-renew.yml) 修改 Cron；GitHub 的定时任务有时会有排队延迟。
